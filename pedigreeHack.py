...
dam_tag_missing_query = "SELECT id AS animal_id, tag_id AS animal_tag_id, sire_id AS father_id, \
                sire_tag_id AS father_tag_id, dam_id AS mother_id, dam_tag_id AS mother_tag_id \
                FROM core_animal WHERE dam_tag_id != '' AND dam_id IS NOT NULL AND dam_tag_id NOT IN (SELECT tag_id FROM core_animal)"
...
sire_tag_missing_query = "SELECT id AS animal_id, tag_id AS animal_tag_id, sire_id AS father_id, \
                sire_tag_id AS father_tag_id, dam_id AS mother_id, dam_tag_id AS mother_tag_id \
                FROM core_animal WHERE sire_tag_id != '' AND dam_id IS NOT NULL AND sire_tag_id NOT IN (SELECT tag_id FROM core_animal)"
# Execute SQL parameterized query to retrieve information for animals where dam tag id != dam id
mismatched_dam_query = "SELECT id AS animal_id, tag_id AS animal_tag_id, sire_id AS father_id, \
                sire_tag_id AS father_tag_id, dam_id AS mother_id, dam_tag_id AS mother_tag_id \
                FROM core_animal WHERE dam_tag_id != '' AND dam_id IS NOT NULL AND dam_tag_id != dam_id"
...
cursor.execute(mismatched_dam_query)
mismatched_dam_results = cursor.fetchall()
df_mismatched_dam = pd.DataFrame(mismatched_dam_results,
                                 columns=['animal_id', 'animal_tag_id', 'father_id', 'father_tag_id', 'mother_id',
                                          'mother_tag_id'])
...
with pd.ExcelWriter('parents_with_most_children.xlsx') as writer:
    ...
    df_mother.to_excel(writer, sheet_name='DamWithMostChildren', index=False)
    ...
    df_mismatched_dam.to_excel(writer, sheet_name='MismatchOfDamTagIDtoDamIDs', index=False)
